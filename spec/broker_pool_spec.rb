describe EbKafka::BrokerPool do
  ...
  before do
    allow(EbKafka::Broker).to receive(:new).and_call_original
  ...
  describe "#connect" do
    it "creates a new broker everytime it is called with node_id nil" do
      ...
      allow(EbKafka::Broker).to receive(:new).once { broker }
      ...
      allow(EbKafka::Broker).to receive(:new).once { second_broker }
    ...
    it "disconnects existing broker if new broker address does not match pre-existing broker address for a given node_id" do
      ...
      allow(EbKafka::Broker).to receive(:new).once
