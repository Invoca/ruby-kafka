describe "Consumer API", functional: true do
  ...
  example "consuming messages from the beginning of a topic" do
    ...
    begin
      kafka = EbKafka.new(kafka_brokers, client_id: "test")
    ...
    consumers = 2.times.map do
      kafka = EbKafka.new(kafka_brokers, client_id: "test", logger: logger)
  ...
  example "consumers process all messages in-order and non-duplicated" do
    ...
    begin
      kafka = EbKafka.new(seed_brokers: kafka_brokers, client_id: "test")
      producer = kafka.producer
    ...
    consumers = 2.times.map do
      kafka = EbKafka.new(seed_brokers: kafka_brokers, client_id: "test", logger: logger)
      consumer = kafka.consumer(group_id: group_id)
      consumer.subscribe(topic)
      consumer
    end
